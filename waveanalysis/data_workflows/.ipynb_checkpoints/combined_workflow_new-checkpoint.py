...
def combined_workflow(
    ...
    *,                                # ← everything after here is keyword‐only
    ...
    #list of array keys instead of TIFF files
    file_names = list(image_arrays.keys())
    ...
    with tqdm(total = len(file_names)) as pbar:
        ...
        for file_name in file_names: 
            try:
                ...
                image_array = image_arrays[file_name]
                metadata    = image_metadata[file_name]
                
                if image_array.ndim == 3:
                    # your case: (time, H, W) → single channel
                    num_frames, height, width = image_array.shape
                    num_channels = 1
                    image_array = image_array.reshape(num_frames, 1, height, width)
                elif image_array.ndim == 4:
                    # already: (time, channels, H, W)
                    num_frames, num_channels, height, width = image_array.shape
                else:
                    raise ValueError(f"{file_name}: expected 3-D or 4-D array, got shape {image_array.shape}")
                ...
                # --- build img_props_dict exactly as before ---
                img_props_dict = {
                ...
                # check & possibly correct the frame interval
                img_props_dict['frame_interval'] = hf.check_frame_interval(
                ...
                # add your other parameters exactly as before
                img_props_dict.update({
                    'step':          bin_shift,
                ...
                # log_params['Frame Interval'].append(f"{file_name}: {img_props_dict['frame_interval']} seconds")
                    ...
                    # logging
                log_params['Pixel Size'].append(
                    f"{file_name}: {img_props_dict['pixel_size']} {img_props_dict['pixel_unit']}"
                    )
                log_params['Frame Interval'].append(
                    f"{file_name}: {img_props_dict['frame_interval']} s"
                    )

                    # ensure enough frames for standard analysis
                if analysis_type=='standard' and num_frames < 11:
                    print(f"ERROR: {file_name} has only {num_frames} frames; need ≥11.")
                    log_params['Files Not Processed'].append(file_name)
                    log_params['Errors'].append(f"Too few frames: {num_frames}")
                    continue
                ...
                # now call your binning exactly as before:
                if analysis_type == 'standard':
                    bin_values, num_bins, _, _ = create_multi_frame_bin_array(
                        image=image_array, img_props=img_props_dict
                        )
                else:
                ...
                img_props_dict['channel_combos'] = channel_combos
                img_props_dict['num_combos']     = len(channel_combos)
                img_props_dict['num_bins']       = num_bins
                img_props_dict['bin_values']     = bin_values
